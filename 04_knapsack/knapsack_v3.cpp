...
Population crossover(const Population& parents) {
    ...
    int idx = rand(0, chromosomeLength - 1);
...
void mutate(Population& offspring) {
    ...
    for (int i = 0; i < offspring.size(); i++) {
        ...
        float pmutation = PMUTATION * rand(0, 1 / PMUTATION);
...
int main() {
    ...
    cout << "Best solution value: " << getFitness(bestSolution) << endl;
    cout << "Best solution weight: " << getWeight(bestSolution) << endl;
